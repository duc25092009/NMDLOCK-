...
class AuthRepository @Inject constructor(
    ...
    suspend fun login(username: String, password: String): Result<AuthResponse> {
        return try {
            ...
            if (response.isSuccessful && body?.success == true) {
                val data = body.data ?: return Result.failure(Exception(body.message ?: "Login response is empty"))
                // Store tokens
    ...
    suspend fun refreshToken(): Result<RefreshResponse> {
        return try {
            ...
            val body = response.body()
            if (response.isSuccessful && body?.success == true) {
                val data = body.data ?: return Result.failure(Exception(body.message ?: "Refresh response is empty"))
                AuthTokenHolder.token = data.accessToken
                AuthTokenHolder.refreshToken = data.refreshToken
                dataStoreManager.saveTokens(data.accessToken, data.refreshToken)
                Result.success(data)
