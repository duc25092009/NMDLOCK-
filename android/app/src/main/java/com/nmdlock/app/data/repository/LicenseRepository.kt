...
class LicenseRepository @Inject constructor(
    ...
    suspend fun activateLicense(keyValue: String): Result<ActivateLicenseResponse> {
        return try {
            ...
            if (response.isSuccessful && body?.success == true) {
                val data = body.data ?: return Result.failure(Exception(body.message ?: "Activation response is empty"))
                ...
                val msg = body?.message ?: "Activation failed"
    ...
    suspend fun validateLicense(): Result<ValidateLicenseResponse> {
        return try {
            ...
            val body = response.body()
            if (response.isSuccessful && body?.success == true) {
                val data = body.data ?: return Result.failure(Exception(body.message ?: "Validation response is empty"))
    ...
    suspend fun getMyLicense(): Result<LicenseInfoResponse> {
        return try {
            ...
            val body = response.body()
            if (response.isSuccessful && body?.success == true) {
                val data = body.data ?: return Result.success(
                    LicenseInfoResponse(
                        active = false,
                        message = body.message ?: "No active license",
                    )
                )
                dataStoreManager.setFullLicenseCache(
    ...
    suspend fun getHistory(): Result<List<LicenseHistoryEntry>> {
        return try {
            ...
            val body = response.body()
            if (response.isSuccessful && body?.success == true) {
                Result.success(body.data?.items ?: emptyList())
            } else {
